...
| D13 | **Per-app routing (opt-in, off by default).** A second routing axis on the originating app, alongside the hostname allow-list. Identity is resolved per connection by a `libproc` scan (`LOCAL_PEERPID` is AF_UNIX-only) and attributed to the app bundle / responsible process; rules key on bundle id, executable path, or executable name. Each app (and the default for unlisted apps) picks one of three modes: **Tunnel all / Use target rules / Direct all**. `ProxyServer` scans only when `RoutingEngine.needsAppIdentity`. Implemented: identity + routing + telemetry + Settings → Apps UI + menu-bar front-app control. See `docs/per-app-rules.md`. | ✅ Confirmed |
