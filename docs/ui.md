...
- The menu bar item is a **native AppKit `NSStatusItem` + `NSMenu` (`StatusMenuController`)** — not a SwiftUI `MenuBarExtra`. A `.menu`-style SwiftUI extra cannot tint status rows or show native key equivalents. **Interactive rows are plain `NSMenuItem`s** (native hover highlight + native checkmark); only the inert status/error rows are custom views (for the colored dots):
  ...
  - **Open Dashboard… ⌘D / Settings… ⌘, / About / Quit ⌘Q** — key equivalents are displayed on the items; while the app is active the real ⌘Q/⌘,/⌘D handling comes from the SwiftUI main menu (`Settings` scene + `AppCommands`).
...
- Dashboard: header (toggle, route/host filters, pause, clear), stat boxes, a live **request-activity chart** (last 10 min, 30 s buckets, stacked by route, computed from the in-memory feed — no SQLite lag), top tunneled hosts (also in-memory), and the request feed as a table with column headers (Time/Route/Method/Host/Status/Bytes/Duration/Error).
...
- Rows are pushed into the table via `NSViewRepresentable`; on change it does a full `reloadData()` only when the row *count* changes, otherwise it refreshes just the on-screen rows (live durations/bytes tick at 10 Hz — no per-cell work off-screen).
...
- `filteredRequests` materializes only the live list + `suffix(250)` of the completed feed, then filters/sorts to ≤250 rows.

## Settings (`SettingsView.swift`)

A sidebar layout — a fixed-width `List` (left) + `Divider` + detail pane (right) — driven by `SettingsSection` (order: **General / Tunnel / Proxy / Targets / System / Monitoring**). Every page opens with a `SettingsPage` header (title + one-line description) so each tab is self-explanatory. Help is a **`HelpPopover`** (`Sources/UI/HelpPopover.swift`, inline `?` → popover with explanation + optional `example`), used **sparingly** — only on genuinely non-obvious options (tunnel host/port, SSH auth, fail-open/closed, terminal-app env, wildcard matching); everything else uses short captions.
